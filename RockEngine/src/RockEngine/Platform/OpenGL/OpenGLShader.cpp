...
#define RE_LOG_UNIFORM
	...
	std::string OpenGLShader::ReadShaderFromFile(const std::filesystem::path& filepath) const
	{
		...
		{
			RE_CORE_ASSERT(false, "Could not load shader!");
		}
		in.close();
		return result;
	}
	...
	void OpenGLShader::ParseUniform(const std::string& statement, ShaderDomain domain)
	{
		...
		{
			ShaderResourceDecl* declaration = new OpenGLShaderResourceDecl(OpenGLShaderResourceDecl::StringToType(typeString), name, count);
			m_Resources.push_back(declaration);
		}
		else
		{
			OpenGLShaderUniformDecl::Type t = OpenGLShaderUniformDecl::StringToType(typeString);
			OpenGLShaderUniformDecl* declaration = nullptr;
			...
			if (t == OpenGLShaderUniformDecl::Type::NONE)
			{
				...
				RE_CORE_ASSERT(s, "");
				declaration = new OpenGLShaderUniformDecl(domain, s, name, count);
			}
			else
			{
				declaration = new OpenGLShaderUniformDecl(domain, t, name, count);
			}

			if (StartsWith(name, "r_"))
			{
				if (domain == ShaderDomain::Vertex)
					((OpenGLShaderUniformBufferDecl*)m_VSRendererUniformBuffers.front())->PushUniform(declaration);
				else if (domain == ShaderDomain::Pixel)
					((OpenGLShaderUniformBufferDecl*)m_PSRendererUniformBuffers.front())->PushUniform(declaration);
			}
			else
			{
				if (domain == ShaderDomain::Vertex)
	...
	int32_t OpenGLShader::GetUniformLocation(const std::string& name) const
	{
		...
		if (result == -1)
			RE_CORE_WARN("Could not find uniform '{0}' in shader {1}", name, m_Name);
